...
public class Fq implements FieldElement<Fq> {
  ...
  /**
   * To a big-endian, unsigned byte array of the field element.
   *
   * @return the bytes
   */
  public byte[] toBytes() {
    final byte[] raw = n.toByteArray();
    // Strip a single leading sign byte if BigInteger added one for a positive value.
    if (raw.length > 1 && raw[0] == 0) {
      final byte[] trimmed = new byte[raw.length - 1];
